...
import { FileOutput, ArrowRight, Download, Loader2, CheckCircle, AlertCircle, FileText, Trash2, RefreshCw } from 'lucide-react';
...
export default function PdfToWordPage() {
    ...
    const handleConvert = async () => {
        ...
        try {
            const response = await api.pdfToWord(file.id);
        ...
        <div className="min-h-screen py-12 bg-gradient-to-b from-indigo-50 to-white">
            <div className="max-w-4xl mx-auto px-4">
                ...
                <div className="text-center mb-10">
                    ...
                    <h1 className="text-3xl font-bold text-slate-900">PDF to Word</h1>
                    <p className="mt-3 text-lg text-slate-600">
                        Convert PDF to editable DOCX documents
                    </p>
                </div>
                ...
                <div className="bg-white rounded-3xl shadow-xl overflow-hidden border border-slate-100">
                    <div className="p-8">
                        {state === 'idle' && !file && (
                            <div
                                {...getRootProps()}
                                className={`border-2 border-dashed rounded-2xl p-10 text-center cursor-pointer transition-colors ${isDragActive ? 'border-indigo-500 bg-indigo-50' : 'border-slate-200 hover:border-indigo-500 hover:bg-slate-50'
                                    }`}
                            >
                                <input {...getInputProps()} />
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="flex justify-center">
                                    <button
                                        ...
                                        className="flex items-center gap-2 px-8 py-4 bg-indigo-600 text-white rounded-xl font-semibold hover:bg-indigo-700 transition-colors shadow-lg shadow-indigo-500/25"
                                    >
                                        Convert to Word
                        ...
                        {state === 'completed' && result && (
                            <div className="text-center py-8">
                                ...
                                <p className="text-slate-500 mb-8">Your Word document is ready</p>

                                <div className="flex flex-col sm:flex-row gap-4 justify-center">
                                    <button
                                        onClick={handleDownload}
                                        className="flex items-center justify-center gap-2 px-8 py-4 bg-indigo-600 text-white rounded-xl font-semibold hover:bg-indigo-700 transition-colors shadow-lg shadow-indigo-500/25"
                                    >
                ...
                <div className="mt-8 text-center">
                    <Link href="/" className="text-slate-500 hover:text-indigo-600 font-medium transition-colors">
                        ← Back to all tools
                    </Link>
                </div>
            </div>
